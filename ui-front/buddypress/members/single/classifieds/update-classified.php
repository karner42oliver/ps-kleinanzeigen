...
if(! isset($_REQUEST['post_id']) ){
	...
	$post_ID = wp_insert_post( array( 'post_title' => __( 'Automatischer Entwurf', $this->text_domain ), 'post_type' => 'classifieds', 'post_status' => 'auto-draft', 'comment_status' => 'closed', 'ping_status' => 'closed'), true );
...
<div class="cf_update_form">
	...
	<form class="standard-form base" method="post" action="#" enctype="multipart/form-data" id="cf_update_form" >
		...
		<div class="editfield">
			...
			<p class="description"><?php _e( 'Gib hier einen passenden Titel ein.', $this->text_domain ); ?></p>
		</div>
		<?php endif; ?>
		...
		<div class="editfield">
			...
			<script type="text/javascript">js_translate.image_chosen = '<?php _e("Beitragsbild ausgewaehlt", $this->text_domain); ?>';</script>
			<span class="upload-button">
				...
				<button type="button" class="button"><?php _e('Beitragsbild festlegen', $this->text_domain); ?></button>
			</span>
			<br />
		...
		<p class="description"><?php _e( 'Beschreib kurz und klar, was du anbietest.', $this->text_domain ); ?></p>
		<?php endif; ?>

		<?php if(post_type_supports('classifieds','excerpt') ): ?>
		<div class="editfield alt">
			<label for="excerpt"><?php _e( 'Kurztext', $this->text_domain ); ?></label>
			...
			<p class="description"><?php _e( 'Optional: ein kurzer Teasertext fuer die Anzeige.', $this->text_domain ); ?></p>
		</div>
		<?php endif; ?>
		...
		<div class="editfield" >
			...
			<p class="description"><?php _e( 'Waehle den Status fuer deine Anzeige.', $this->text_domain ); ?></p>
		</div>

		<?php
		$duration_meta_key = '_cf_duration';
		$cost_meta_key = '_cf_cost';
		$duration_value = ! empty( $classified_data['ID'] ) ? get_post_meta( $classified_data['ID'], $duration_meta_key, true ) : '';
		$cost_value = ! empty( $classified_data['ID'] ) ? get_post_meta( $classified_data['ID'], $cost_meta_key, true ) : '';
		...
		<div class="editfield">
			...
			<select id="<?php echo esc_attr( $duration_meta_key ); ?>" name="<?php echo esc_attr( $duration_meta_key ); ?>">
				<option value=""><?php _e( 'Bitte waehlen', $this->text_domain ); ?></option>
				<?php foreach ( $duration_options as $duration_option_value => $duration_option_label ): ?>
				<option value="<?php echo esc_attr( $duration_option_value ); ?>" <?php selected( $duration_value, $duration_option_value ); ?>><?php echo esc_html( $duration_option_label ); ?></option>
				<?php endforeach; ?>
			</select>
		...
		<div class="editfield">
			...
			<input type="text" id="<?php echo esc_attr( $cost_meta_key ); ?>" name="<?php echo esc_attr( $cost_meta_key ); ?>" value="<?php echo esc_attr( $cost_value ); ?>" />
		...
		<div class="submit">
			...
			<input type="submit" value="<?php _e( 'Speichern', $this->text_domain ); ?>" name="update_classified">
